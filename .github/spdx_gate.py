...
def main(argv: list[str]) -> int:
    ...
    # ⚠️ THROUGH THE SAME HELPER AS EVERY OTHER GIT CALL. This was a fourth,
    # hand-rolled copy of `_git_z_all` sitting inside `main`.
    ok_all, all_names = _git_z_all(root)
    if not ok_all:
        # ⚠️ AND THE OLD FORM PASSED SILENTLY HERE. `stale_ack = ... if
        # tracked_all else []` meant a FAILED enumeration produced an empty
        # set, which produced an empty stale list, which is the PASS
        # condition - the same vacuity this gate has now fixed three times.
        print("FAIL: could not enumerate the tree to check "
              "COPYRIGHT_ACKNOWLEDGED.", file=sys.stderr)
        return 1
    tracked_all = set(all_names)
    stale_ack = sorted(set(COPYRIGHT_ACKNOWLEDGED) - tracked_all)
